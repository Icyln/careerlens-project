...
function ResumeSelector({ resumes, selectedResumeId, setSelectedResumeId }) {
  if (!resumes.length) {
    return (
      <div className="rounded-2xl border border-dashed border-[#111439]/20 bg-white/50 p-6 sm:p-8 text-center backdrop-blur-sm">
        ...
        <Link to="/resumes" className="mt-5 inline-flex items-center justify-center rounded-xl bg-[#111439] px-6 py-3 text-sm font-semibold text-white hover:bg-[#1a1f54] transition-all shadow-md w-full sm:w-auto">
          Upload Resume
        </Link>
      </div>
    );
  ...
  if (resumes.length === 1) {
    ...
    return (
      <div className="rounded-2xl bg-white p-5 sm:p-6 border border-[#111439]/5 shadow-sm">
...
function AiList({ title, items = [], empty = 'Nothing to show here.' }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 sm:p-6 border border-[#111439]/5 shadow-sm h-full">
...
function hasTechnicalAiText(ai) {
  const text = [
    ai?.status,
    ai?.message,
    ai?.summary_10_second_read,
    ai?.alignment_explanation?.explanation,
    ...toArray(ai?.recommendations),
  ]
...
function isEnhancedReviewUnavailable(ai) {
  if (!ai) return true;

  const status = toText(ai?.status, '').toLowerCase();

  return (
    ['error', 'unavailable', 'fallback', 'temporarily_unavailable'].includes(status)
    || hasTechnicalAiText(ai)
  );
}

function EnhancedReviewFallbackCard() {
  return (
    <div className="mt-8 md:mt-12 rounded-[1.5rem] md:rounded-[2.5rem] bg-gradient-to-b from-white to-[#F8F8F9] p-6 sm:p-10 shadow-xl shadow-[#111439]/5 border border-[#111439]/10 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[300px] md:w-[520px] h-[300px] md:h-[520px] bg-gradient-to-bl from-amber-100 via-blue-50 to-transparent rounded-full blur-[70px] pointer-events-none" />

      <div className="relative z-10">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white shadow-sm border border-amber-100 text-xs font-semibold uppercase tracking-widest text-amber-700 mb-4">
          <BrainCircuit size={16} />
          Recruiter Review
        </div>

        <h3 className="text-2xl sm:text-3xl font-semibold text-[#111439]">
          Enhanced review is temporarily unavailable
        </h3>

        <p className="mt-3 max-w-3xl text-base leading-relaxed text-[#111439]/75 font-medium text-justify">
          Your ATS scan was completed successfully. The score, checklist, and recommendations are ready, so you can continue improving your resume without waiting for the optional recruiter-style review.
        </p>

        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
            <CheckCircle2 className="text-[#0D9476]" size={20} />
            <p className="mt-3 text-sm font-semibold text-[#111439]">ATS report completed</p>
            <p className="mt-1 text-xs leading-5 text-[#111439]/60">
              Your score and detailed checklist are available.
            </p>
          </div>

          <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
            <ListChecks className="text-[#106EBE]" size={20} />
            <p className="mt-3 text-sm font-semibold text-[#111439]">Use the top fixes</p>
            <p className="mt-1 text-xs leading-5 text-[#111439]/60">
              Start with the recommendations from the rule-based ATS engine.
            </p>
          </div>

          <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
            <AlertTriangle className="text-amber-600" size={20} />
            <p className="mt-3 text-sm font-semibold text-[#111439]">Try again later</p>
            <p className="mt-1 text-xs leading-5 text-[#111439]/60">
              The optional review can be retried after editing or running a new scan.
            </p>
          </div>
        </div>
      </div>
...
function AiAnalysisPanel({ ai }) {
  ...
  if (isEnhancedReviewUnavailable(ai)) {
    return <EnhancedReviewFallbackCard />;
  }

  return (
    <div className="mt-8 md:mt-12 rounded-[1.5rem] md:rounded-[2.5rem] bg-gradient-to-b from-white to-[#F8F8F9] p-6 sm:p-12 shadow-xl shadow-[#111439]/5 border border-[#111439]/10 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-gradient-to-bl from-[#0FFCBE]/20 via-[#106EBE]/10 to-transparent rounded-full blur-[60px] md:blur-[80px] pointer-events-none" />

      <div className="relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-8 sm:mb-10 border-b border-[#111439]/5 pb-6 sm:pb-8">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 text-base leading-relaxed text-[#111439]/80 font-medium text-justify">
              {toText(ai.summary_10_second_read, 'Enhanced recruiter-style insights were generated for this scan.')}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1.1fr]">
          <div className="space-y-6 text-justify">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#111439]/5 shadow-sm">
              <h4 className="text-sm font-semibold uppercase tracking-widest text-[#111439]/50 mb-3">
                Overall Alignment
              </h4>
              <p className="text-2xl sm:text-3xl font-extrabold text-[#106EBE] mb-3">
                {toText(ai.alignment_explanation?.level, 'Analyzed')}
              </p>
              <p className="text-sm text-[#111439]/70 leading-relaxed font-medium">
                {toText(ai.alignment_explanation?.explanation, 'No detailed explanation provided.')}
              </p>
            </div>

            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#111439]/5 shadow-sm">
              <h4 className="text-sm font-semibold uppercase tracking-widest text-[#111439]/50 mb-6">
                Recruiter Metric Breakdown
              </h4>
              <div className="space-y-6">
                {(toArray(ai.visualization).length
                  ? toArray(ai.visualization)
                  : [{ label: 'Role Fit Overview', value: 0, reason: 'No metric breakdown returned.' }]
                ).map((item, index) => (
                  <ProgressBar
                    key={`ai-viz-${index}`}
                    value={item?.value}
                    label={displayValue(item?.label)}
                    caption={displayValue(item?.reason)}
                  />
                ))}
              </div>
            </div>
...
function ScoreCard({ card }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 sm:p-6 border border-[#111439]/5 shadow-sm flex flex-col h-full">
      ...
      <p className="text-3xl sm:text-4xl font-semibold text-[#111439]">{score}%</p>
...
function IssueGroupCard({ group, icon }) {
  ...
  const defaultSuccessFeedback = (() => {
    if (groupName === 'Searchability') return 'Core recruiter contact details were detected and this section has no unresolved issues.';
    if (groupName === 'Job Title Match') return 'The target job title was found in the resume, so this section has no detected issue.';
    if (groupName === 'Experience Years') return 'The resume experience appears to satisfy the experience requirement detected from the job description.';
    if (groupName === 'Education Match') return 'The resume education section matches the education requirement detected from the job description.';
    if (groupName === 'Hard Skills') return 'The hard-skill requirements detected from the job description are covered in the resume.';
    if (groupName === 'Soft Skills') return 'The soft-skill requirements detected from the job description are covered in the resume.';
    if (groupName === 'Formatting') return 'No formatting issues were detected. The uploaded file looks readable for ATS parsing.';
    if (groupName === 'Recruiter Tips') return 'No recruiter-review issues were detected for this section.';
  ...
  return (
    <div className="rounded-2xl bg-white p-5 sm:p-6 border border-[#111439]/5 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl bg-[#F8F8F9] p-4 border border-[#111439]/5 overflow-hidden">
...
export default function ATSAnalysisPage() {
  ...
  const handleAnalyze = async (event) => {
    ...
    try {
      ...
      setMessage({
       type: 'success',
       text: 'Scan complete. Your ATS score, checklist, and recommendations are ready.',
      });
  ...
  return (
    <div className="space-y-6 md:space-y-8 relative font-['Inter',_ui-sans-serif,_system-ui,_sans-serif] text-[#111439] min-h-screen pb-16 pt-6 px-4 md:pt-0 md:px-0 z-0 selection:bg-[#0FFCBE]/30 overflow-x-hidden">
      ...
      {/* Header Section */}
      <section className="bg-white border border-white rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-10 shadow-xl shadow-[#111439]/5">
        <div className="flex flex-col gap-6 lg:gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-2xl sm:text-3xl font-semibold tracking-tight text-slate-950 mb-3">
          ...
          {/* Feature cards */}
          <div className="grid grid-cols-2 sm:flex sm:flex-row gap-3 sm:gap-4 sm:w-auto w-full">
            <div className="flex-1 lg:w-48 rounded-2xl bg-white border border-[#111439]/5 p-4 sm:p-6 shadow-sm hover:shadow-md hover:border-[#106EBE]/20 transition-all">
              <ListChecks className="text-[#106EBE] mb-2 sm:mb-3" size={28} />
              <p className="text-sm sm:text-base font-bold text-[#111439]">ATS Check</p>
              <p className="text-xs text-[#111439]/60 mt-1 sm:mt-1.5 font-medium">Finds exact keywords</p>
            </div>
            <div className="flex-1 lg:w-48 rounded-2xl bg-white border border-[#111439]/5 p-4 sm:p-6 shadow-sm hover:shadow-md hover:border-[#0FFCBE]/30 transition-all">
              <BrainCircuit className="text-[#0D9476] mb-2 sm:mb-3" size={28} />
              <p className="text-sm sm:text-base font-bold text-[#111439]">Smart Review</p>
              <p className="text-xs text-[#111439]/60 mt-1 sm:mt-1.5 font-medium">Reads like a recruiter</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Input Form Section */}
      <form onSubmit={handleAnalyze} className="grid gap-6 lg:gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="space-y-6">
          {loadingResumes ? (
          ...
          {selectedResume && (
            <div className="rounded-2xl bg-[#111439] p-5 sm:p-6 text-white shadow-xl shadow-[#111439]/10 relative overflow-hidden">
        ...
        <div className="rounded-3xl bg-white/80 backdrop-blur-md p-5 sm:p-8 shadow-sm border border-[#111439]/5">
          <div className="grid gap-5 sm:gap-6">
            <div>
              <label htmlFor="job-title" className="block text-xs font-semibold uppercase tracking-widest text-[#111439]/70 mb-2 sm:mb-3 pl-1">Target Job Title</label>
              <input
                id="job-title"
                type="text"
                value={jobTitle}
                ...
                className="w-full rounded-2xl border border-[#111439]/10 bg-[#F8F8F9] px-4 py-3 sm:px-5 sm:py-4 text-base font-medium text-[#111439] placeholder-[#111439]/30 focus:border-[#106EBE] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#106EBE]/10 transition-all duration-300"
              />
            </div>

            <div>
              <label htmlFor="job-description" className="block text-xs font-semibold uppercase tracking-widest text-[#111439]/70 mb-2 sm:mb-3 pl-1">Job Description</label>
              <textarea
                id="job-description"
                ...
                rows={8}
                className="w-full resize-y rounded-2xl border border-[#111439]/10 bg-[#F8F8F9] px-4 py-3 sm:px-5 sm:py-4 text-base leading-relaxed text-[#111439] placeholder-[#111439]/30 focus:border-[#106EBE] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#106EBE]/10 transition-all duration-300 custom-scrollbar"
              />
            </div>
            ...
            <button
              ...
              className="relative w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-400 px-6 py-3 sm:py-4 text-base font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:scale-100 group overflow-hidden"
            >
      ...
      {/* RESULTS SECTION */}
      {report && ats && (
        <section ref={reportRef} className="pt-8 space-y-6 md:space-y-8" id="analysis-report">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#111439]/5 pb-6">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl border border-[#111439]/10 bg-white px-5 py-3 text-sm font-semibold text-[#111439] shadow-sm transition hover:bg-[#F8F8F9] hover:-translate-y-0.5"
              >
                <Search size={16} />
                Run New Scan
              </button>
              ...
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-[#111439] px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-[#111439]/20 transition hover:bg-[#1a1f54] hover:-translate-y-0.5"
              >
